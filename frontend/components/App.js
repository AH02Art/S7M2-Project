...
export default function App() {
  ...
  const emptyForm = { fname: "", lname: "", bio: "" };
  ...
  useEffect(() => {
    ...
    if (editing === null) {
      ...
      const { fname, lname, bio } = members.find((items) => items.id === editing);
  ...
  const onChange = evt => {
    ...
    setValues(previousValues => ({ ...previousValues, [id]: value }));
  }
  const edit = id => {
  ...
  const submitNewMember = () => {
    ...
    const addedMember = { fname, lname, bio, id: getId() };
  ...
  const editExistingMember = () => {
    ...
    setMembers(previousMembers => previousMembers.map((items) => {
      if (items.id === editing) {
        return { ...items, ...values }
      }
      return items;
        ...
        <div>
          {
            members.map(mem => (
              <div key={mem.id} className="member">
                ...
                <button onClick={function() {edit(mem.id)}}>Edit</button>
              </div>
            ))
          }
        </div>
      ...
      <div id="membersForm">
        ...
        <form onSubmit={onSubmit}>
          <div>
            ...
            <input value={values.fname} id="fname" type="text" placeholder="Type First Name" onChange={onChange} />
          </div>

          <div>
            <label htmlFor="lname">Last Name </label>
            <input value={values.lname} id="lname" type="text" placeholder="Type Last Name" onChange={onChange} />
          </div>

          <div>
            <label htmlFor="bio">Bio </label>
            <textarea value={values.bio} id="bio" placeholder="Type Bio" onChange={onChange} />
          </div>
